...
// $Endlog$
/*
 * Copyright (c) 1997, 1998 Systemics Ltd on behalf of the Cryptix Development
...
public class Twofish_Properties // implicit no-argument constructor
{
    ...
    static {
        ...
        if (!ok) {
            if (GLOBAL_DEBUG)
                System.err
                        .println(">>> " + NAME + ": WARNING: Unable to load \"" + it + "\" from CLASSPATH.");
            if (GLOBAL_DEBUG)
    ...
    /**
     * Get the value of a property for this algorithm, or return <i>value</i> if
     ...
     * <p>
     * 
     * User indicates this by setting the tracing <code>boolean</code> property
     * for <i>label</i> in the <code>(algorithm).properties</code> file. The
     * property's key is "<code>Trace.<i>label</i></code>".
     * <p>
     * 
     ...
     * next. If neither property is set, or if the first property found is not a
     * valid decimal integer, then this method returns 0.
     * 
     * @param label The name of a class.
